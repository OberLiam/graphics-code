...
public class Sphere implements IIntersector{
	Vector3 center;
	double radius;
	Vector3 colorVec;
	...
	public Intersection getIntersection(Ray incoming) {
		...
		if(d2 >= Intersection.BUFFER && d1 < Intersection.BUFFER) // the intersection is @d2
			return Intersection.make(d2, incoming, Vector3.sub(incoming.extrapolate(d2),center).normalized(), colorVec);
		else if(d1 >= Intersection.BUFFER) // the intersection is @d1
			return Intersection.make(d1, incoming, Vector3.sub(incoming.extrapolate(d1),center).normalized(), colorVec);
		else
			return null;
	}
}