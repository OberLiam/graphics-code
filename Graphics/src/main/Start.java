...
public class Start
{
	...
	public static final int DIM_WIDTH = 720;
	public static final int DIM_HEIGHT = 720;
	...
	public static void buildWorldInfinity(Cosmos theCosmos) {
		...
		sphereB.intersector = new Sphere(
				new Vector3(+0.15, -0.5, -1.2),
				0.4,
				Vector3.GREEN);
		
		RealThing floor = new RealThing();
		floor.reflective = true;
		floor.reflectivity = 1.00;
		floor.transparent = false;
		floor.intersector = new Plane(
				new Vector3(0.0, 0.3, 0.0), //p
				new Vector3(1.0, 0.0, 0.0), //u
				new Vector3(0.0, 0.0, 1.0), //v
				IShape.PLANE,
				Vector3.GREEN);
		...
		theCosmos.realThings = new RealThing[] {sphereL, sphereR, sphereB, floor};
		theCosmos.lights = new Light[] {light};
	}
