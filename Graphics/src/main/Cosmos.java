...
public class Cosmos {
	...
	public Light[] lights;
	private final double fogEffect = 0.01;
	...
	public final double ambientIntensity = 0.1;
	...
	public Vector3 getColorVector(Ray specific, int numInducts) {
		...
		if(applyLights) {
			double intensity = ambientIntensity; // starts @ ambientIntensity
			for(Light light : lights) {
				double plusIntensity = light.luminosity;
				double transparency = getNetTransparency(bestInter.position, light.position);
				plusIntensity *= transparency; // blocking
				Vector3 displacement = Vector3.sub(light.position, bestInter.position);
				plusIntensity *= lightFunction(displacement.length()); // faaaade
				plusIntensity *= Math.pow(Vector3.dot(bestInter.normal, displacement.normalized()), 2); // slant lighting //NOTE: don't pow this
				intensity += plusIntensity;
			}
